#include "Socket.h"
#include <iostream>
#include <vector>
#include <sys/epoll.h>
#include <algorithm>
#include <fcntl.h>
#include <errno.h>

// 클라이언트 이벤트?
constexpr int MAX_EVENTS = 10;

void set_nonblocking(int fd)
{
    int flags = fcntl(fd, F_GETFL, 0);
    if (flags < 0)
    {
        throw std::runtime_error("fcntl F_GETFL failled!");
    }
    if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) < 0)
    {
        throw std::runtime_error("fcntl F_SETFL failled!");
    }
}

int main()
{
    try
    {
        std::cout << "=== EPOLL(ET) 기반 ECHO Server 시작 ===\n";

        // ServerSocket으로 서버 초기화 (생성/바인딩/리슨 자동)
        ServerSocket server(8080);

        // 서버 소켓도 논블로킹으로 설정
        set_nonblocking(server.get_fd());

        // Epoll 인스턴스 생성
        int epoll_fd = epoll_create1(0);
        if (epoll_fd < 0)
        {
            throw std::runtime_error("epoll_create1 실패");
        }

        // 서버 소켓을 epoll에 등록
        epoll_event serverEv;
        serverEv.events = EPOLLIN | EPOLLET;
        serverEv.data.fd = server.get_fd();

        // 0 미만이면 실패
        if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, server.get_fd(), &serverEv) < 0)
        {
            throw std::runtime_error("epoll_ctl 실패");
        }

        // 클라이언트 소켓 목록 배열 -> pollfd와 동기화
        std::vector<SocketHandle> clients;

        // 이벤트 배열 (epoll_wait 결과를 받을 배열)
        epoll_event events[MAX_EVENTS];

        // 메인 루프
        while (true)
        {
            std::cout << "epoll_wait() 대기 중... (현재 클라이언트: "
                      << clients.size() << "명)\n";

            // 이벤트 대기 (한번만 호출)
            int nfds = epoll_wait(epoll_fd, events, MAX_EVENTS, -1);

            if (nfds < 0)
            {
                throw std::runtime_error("epoll_wait 실패");
            }

            // 발생한 이벤트만 순회
            for (size_t i = 0; i < nfds; i++)
            {
                int fd = events[i].data.fd; // 서버, 클라 가리지 않는 fd

                std::cout << "[fd=" << fd << "] events: ";
                if (events[i].events & EPOLLIN)
                    std::cout << "EPOLLIN ";
                if (events[i].events & EPOLLHUP)
                    std::cout << "EPOLLHUP ";
                if (events[i].events & EPOLLERR)
                    std::cout << "EPOLLERR ";
                std::cout << "\n";

                if (events[i].events & (EPOLLHUP | EPOLLERR))
                {
                    std::cout << "클라이언트 " << fd << " 비정상 종료 (HUP/ERR)\n";

                    // epoll에서 제거
                    epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, nullptr);

                    auto it = std::find_if(clients.begin(), clients.end(),
                                           [fd](const SocketHandle &s)
                                           { return s.get() == fd; });
                    if (it != clients.end())
                    {
                        clients.erase(it);
                    }

                    // 이 fd는 더 이상 처리할 필요 없음
                    continue;
                }

                if (events[i].events & EPOLLIN)
                {
                    // 직접 fd값과 서버 및 클라의 fd 값을 비교
                    // 서버 소켓: 새 연결
                    if (fd == server.get_fd())
                    {
                        while (true)
                        {
                            try
                            {
                                SocketHandle new_client = server.accept_client();

                                // 새 클라이언트 소켓을 논블로킹으로 설정
                                set_nonblocking(new_client.get());

                                // 새 클라이언트를 epoll에 등록
                                epoll_event client_ev{};
                                client_ev.events = EPOLLIN | EPOLLET;
                                client_ev.data.fd = new_client.get();

                                // epoll_ctl로 새 클라이언트 추가
                                if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, new_client.get(), &client_ev) < 0)
                                {
                                    throw std::runtime_error("클라이언트 epoll_ctl 실패");
                                }

                                clients.push_back(std::move(new_client));
                            }
                            catch (const std::exception &e)
                            {
                                // accept 연결 X (동시 연결 유실 X)
                                if (errno == EAGAIN || errno == EWOULDBLOCK)
                                {
                                    break; // 정상 종료
                                }
                                else
                                {
                                    std::cerr << "accept error: " << e.what() << "\n";
                                    break;
                                }
                            }
                        }
                    }
                    // 클라이언트 소켓: 데이터 수신
                    else
                    {
                        while (true)
                        {
                            std::vector<char> buffer(1024);
                            std::fill(buffer.begin(), buffer.end(), 0);

                            ssize_t bytes = read(fd, buffer.data(), buffer.size() - 1);

                            if (bytes > 0)
                            {
                                // Echo back
                                buffer[bytes] = '\0';
                                std::cout << "[fd=" << fd << "] 받음: " << buffer.data();
                                write(fd, buffer.data(), bytes);
                            }
                            else if (bytes == 0)
                            {
                                // 연결 종료
                                std::cout << "클라이언트 " << fd << " 연결 종료\n";

                                // epoll에서 제거
                                if (epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, nullptr) < 0)
                                {
                                    std::cerr << "epoll_ctl DEL 실패\n";
                                }

                                // clients 벡터에서 찾아서 제거
                                auto it = std::find_if(clients.begin(), clients.end(),
                                                       [fd](const SocketHandle &s)
                                                       { return s.get() == fd; });

                                if (it != clients.end())
                                {
                                    clients.erase(it); // RAII로 자동 close
                                }

                                // while 문 탈출
                                break;
                            }
                            else
                            {
                                if (errno == EAGAIN || errno == EWOULDBLOCK)
                                {
                                    std::cout << "읽기 종료\n";

                                    // while 문 탈출
                                    break;
                                }
                            }
                        }
                    }
                }
            }
        }
    }
    catch (const std::exception &e)
    {
        std::cerr << "에러: " << e.what() << "\n";
        return 1;
    }

    return 0;
}